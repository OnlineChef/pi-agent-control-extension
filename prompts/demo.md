...
---
Runtime note: install this package with `pi install .`; then invoke this as `/demo` and load `pi-agent-control`.


Load skills: **pi-agent-control**.
...
Use the **pi-agent-control** routing tables. Do all three lookups:
...
**Delegation:** For before/after comparisons, capture both branches **in parallel** using worker subagents with `run_in_background=true`. Construct the exact `tctl` commands for each worker (see the delegation section in the pi-agent-control skill). Wait for both to complete before proceeding to compose.
