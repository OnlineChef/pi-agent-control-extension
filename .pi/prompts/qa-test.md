...
---
Runtime note: install this package with `pi install .`; then invoke this as `/qa-test` and load `pi-agent-control`.
...
- **CLI command** (`pi`, `htop`, `my-cli --flag`) → terminal TUI
...
Use the **pi-agent-control** routing tables:
