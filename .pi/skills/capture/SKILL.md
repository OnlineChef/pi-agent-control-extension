...
You should already have a **driver atom** loaded (tuistory, true-input, or agent-browser) and optionally a **target atom** (pi-agent-cli). This atom layers the recording discipline on top.
...
- For `pi` captures, `--repo-root` is **mandatory** — `tctl` will refuse to launch without it
...
# RUN_ID and RUN_DIR should already be set by the parent (see pi-agent-control ground rule 5)
```
