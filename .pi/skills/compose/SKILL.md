---
name: compose
description: Video assembly via Remotion — title cards, layout, transitions, effects, and showcase polish.
---
# Compose
...
```bash
REMOTION_DIR=${PI_AGENT_CONTROL_ROOT}/remotion
RENDER=${PI_AGENT_CONTROL_ROOT}/scripts/render-showcase.sh
```
...
**CRITICAL: `agg` replaces ALL 16 ANSI colors with its theme palette.** The render script uses a custom Pi Coding Agent CLI theme. If you manually run `agg`, never omit `--theme` and never use built-in themes like `monokai` or `dracula`.
